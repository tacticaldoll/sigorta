...
- [x] 1.1 Add a `probe_expires: Instant` field to `State::HalfOpen`.
- [x] 1.2 When `Open` transitions to `HalfOpen` (cooldown elapsed), set
      `probe_expires` to `now + open_duration`.
- [x] 1.3 In `admit`'s `HalfOpen` arm: if `now` is before `probe_expires`, return
...
- [x] 2.1 Update `admission_during_an_outstanding_probe_probes_again` (or split it)
      to cover both the still-within-window and the stale-and-renewed cases.
- [x] 2.2 Run the full Definition of Done.
